...
如果安裝後發現「沒聲音」或連不上「本地司令部」，請檢查 `.env` 檔案：

1. **語音沒聲音？**
   - 確保 `.env` 中有 `FORCE_NATIVE_SAY=true`。這會強制 Mac 使用內建的 `say` 指令，避開套件衝突。
2. **連不到本地司令部？**
   - 確保 `.env` 中的 `MISSION_CONTROL_URL=http://localhost:5010`。
   - 如果是要連遠端正式版，請改回 `https://mission-smart-light.onrender.com`。
